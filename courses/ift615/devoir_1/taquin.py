...
def evaluation(solution_file):
    import pickle
    solutionEtats = pickle.load(open(solution_file))
    #pickle.dump(g_etats, open(solution_file,'w'))
    ...
    for i, (e1, e2) in enumerate(zip(g_etats, solutionEtats), start=1):
        if e1 != e2:
            ...
            print "État #{0}: Votre joueur obtient bien l'état attendu, bravo!".format(i)
    ...
    if len(no_etats_errones) > 0:
        print "Erreur: les états suivants ne correspondent pas à la solution: {0}".format(",".join(map(str, no_etats_errones)))
    else:
...
def buildArgsParser():
    ...
    p.add_argument('-joueur', dest="player", metavar="JOUEUR", action='store', type=str, required=False, default="solution_taquin.py",
                   help="[humain, solution_taquin.py]. Défaut=solution_taquin.py")

    p.add_argument('-no_partie', dest="no_partie", metavar="INT", action='store', type=int, required=False,
                   help="numéro de partie à jouer.")

    p.add_argument('-valider', dest="validation_file", metavar="FICHIER", action='store', type=str, required=False, default='taquin_solution.pkl',
                   help="fichier permettant de valider votre joueur pour un jeu donné. Défaut=taquin_solution.pkl")
...
def main():
    ...
    taquin = Jeu(TaquinEtat(no_partie), taquin_estBut, taquin_transitions, taquin_heuristique, verbose)
